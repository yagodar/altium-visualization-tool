	...
	public PropertyEditorPanel(Browserable browserableObject) {
		...
		if (browserableObject != null) {
			...
			for (Propertiable property : properties) {
				...
				add(new JLabel(name),
						new GBC(0, index).setAnchor(GBC.FIRST_LINE_START)
								.setFill(GBC.BOTH).setInsets(0, 5, 0, 5));
				add(component == null ? new TextFieldViewComponent(property)
						: component,
						new GBC(1, index).setAnchor(GBC.FIRST_LINE_START)
